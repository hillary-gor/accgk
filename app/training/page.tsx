...
import { TrainingEnrollment, TrainingProgram } from "@/types/db.ts";

export default function TrainingProgramsPage() {
  const supabase = useMemo(() => createClient(), []);
  const { user } = useAuth();
  const { toast } = useToast();
  type ProgramWithTrainer = TrainingProgram & {
    trainer?: {
      full_name: string;
    };
  };
  ...
  type EnrollmentWithProgram = TrainingEnrollment & {
        ...
        {isLoading ? (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                There are currently no training programs available.
              </CardDescription>
            </CardHeader>
            {userRole === "trainer" && (
              <CardFooter>
                ...
                <Card key={program.id} className="flex flex-col">
                  ...
                  <CardFooter className="flex flex-col items-stretch gap-2">
                    {enrolled ? (
                      <>
                        <Badge
                          variant={getStatusBadgeVariant(enrollmentStatus)}
