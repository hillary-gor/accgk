...
export default function DataTeamDashboard() {
  ...
  async function handleUpdate(id: string, field: keyof Application, value: string) {
    try {
  ...
  async function handleUpload(id: string, file: File) {
    try {
      ...
      setApplications((prev) =>
        prev.map((app) =>
          app.id === id
            ? { ...app, extra_document_urls: [...(app.extra_document_urls || []), res.fileUrl] }
            : app
        )
      );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      <h1 className="text-2xl font-semibold mb-4 text-gray-800">Data Team Dashboard</h1>
      ...
      {applications.length === 0 ? (
        ...
        <ul className="space-y-4">
          {applications.map((app) => (
            <li key={app.id} className="border p-4 rounded-lg shadow-sm bg-gray-50">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onBlur={(e) => handleUpdate(app.id, "full_name", e.target.value)}
                ...
                <div>
                  ...
                  <input
                    ...
                    onBlur={(e) => handleUpdate(app.id, "phone", e.target.value)}
                    disabled={editing[app.id]}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onBlur={(e) => handleUpdate(app.id, "address", e.target.value)}
                    disabled={editing[app.id]}
                  />
                </div>

                <div>
                  <label className="text-gray-700 font-semibold">Email (Read-Only)</label>
                  <p className="w-full border p-2 rounded bg-gray-100 text-gray-500">{app.email}</p>
                </div>
              </div>
              ...
              <div className="mt-4">
                ...
                <input
                  ...
                  onChange={(e) => e.target.files && handleUpload(app.id, e.target.files[0])}
                />
                {uploading[app.id] && <p className="text-blue-500">Uploading...</p>}
              </div>

              {/* Show Uploaded Documents */}
              {app.extra_document_urls && app.extra_document_urls.length > 0 && (
                <div className="mt-4">
              ...
              <div className="mt-4">
                <button
                  ...
                  onClick={() => setEditing((prev) => ({ ...prev, [app.id]: !prev[app.id] }))}
                >
