...
import { Certification, License } from "@/types/db.ts";
...
export default function VerifyPage() {
  ...
  type VerificationData = License | Certification;
  const [verificationResult, setVerificationResult] = useState<{
    type: "license" | "certification";
    data: VerificationData & {
      user?: { profiles?: { first_name?: string; last_name?: string }[] };
    };
  } | null>(null);
  ...
  const handleVerifyLicense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setVerificationError(
        "Failed to verify license. Please check the ID and try again."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleVerifyCertification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setVerificationError(
        "Failed to verify certification. Please check the ID and try again."
      );
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="flex flex-col items-center justify-center space-y-6">
          ...
          <Card className="w-full max-w-2xl">
            <Tabs
              ...
              onValueChange={(value) =>
                setVerificationType(value as "license" | "certification")
              }
            >
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        The license ID is a unique identifier found on the
                        caregiver's license document
                      </p>
                    </div>
                  </CardContent>
                  <CardFooter>
              ...
              <TabsContent value="certification">
                <form onSubmit={handleVerifyCertification}>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Enter the certification ID to verify its authenticity
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        The certification ID is a unique identifier found on the
                        caregiver's certification document
                      </p>
                    </div>
                  </CardContent>
                  <CardFooter>
            ...
            <Card className="w-full max-w-2xl">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {verificationResult.data.user?.profiles?.[0]
                        ?.first_name || "N/A"}{" "}
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {verificationResult.type === "license"
                        ? getLicenseTypeLabel(
                            verificationResult.data.license_type
                          )
                        : getCertificationTypeLabel(
                            verificationResult.data.certification_type
                          )}
                    </p>
                  </div>
              ...
              <CardFooter className="flex flex-col space-y-2">
                <p className="text-xs text-muted-foreground">
                  ...
                  {new Date().toLocaleString()} and reflects the current status
                  in the ACCGK database.
                </p>
